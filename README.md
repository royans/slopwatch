...
* **Phantom Squatting & Typosquat Detection**: Identifies impersonations of high-value brands (Google, AWS, Stripe, Okta, Clerk, Supabase) using Levenshtein distance, token insertion, and delimiter swap heuristics with brand-entity disambiguation (preventing false alarms on generic terms like `safe` or `base`).
* **Smart Compiler & Build Introspection**: Understands common packaging workflows in `setup.py` / `pyproject.toml` (such as `pip install`, compiler toolchains, `python -c` environment introspection, and formatted subcommands), preventing false alarms on legitimate build shims.
* **Evasion vs. Exfiltration Separation**: Strictly distinguishes network testing shims (e.g. self-signed TLS verification toggles in audit tooling) from confirmed data exfiltration destinations.
* **Application Daemon vs. Backdoor Disambiguation**: Differentiates legitimate application service management (user LaunchAgents/systemd units in developer CLI tools) from shell profile tampering (`~/.bashrc`).
...
* **Respectful of maintainers**: Community libraries with ordinary telemetry, developer service daemons, or standard build shims are evaluated as `BENIGN_COMMUNITY`. The `MALICIOUS` verdict is strictly reserved for confirmed, active weaponization vectors (install-time execution, socket connections, reverse shell spawns, credential exfiltration).
* **Established Community Safeguards**: High-adoption packages (such as libraries with ≥50,000 monthly downloads or mature multi-year codebases) are safeguarded against composite source-code proximity false positives unless backed by confirmed weaponized payloads.
