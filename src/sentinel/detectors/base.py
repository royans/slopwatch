...
logger = logging.getLogger("sentinel.detectors")
...
def _discover_modules() -> None:
    """Import every submodule of ``sentinel.detectors`` so decorators run."""
    import sentinel.detectors as pkg

    for mod in pkgutil.iter_modules(pkg.__path__):
        if mod.name in ("base", "engine") or mod.name.startswith("_"):
            continue
        try:
            importlib.import_module(f"sentinel.detectors.{mod.name}")
...
def _config_disabled_detectors() -> Set[str]:
    ...
    candidates = [
        os.getenv("SENTINEL_DETECTORS_CONFIG"),
