...
def _find_default_signatures_path() -> str:
    ...
    try:
        ...
        sig_pkg = pkg_resources.files("sentinel") / "signatures" / "parking_hashes.json"
        if sig_pkg.is_file():
            return str(sig_pkg)
    except Exception:
        pass
...
class Settings(BaseModel):
    ...
    def load(cls, config_path: Optional[str] = None) -> "Settings":
        ...
        path_to_try = config_path or os.getenv("SENTINEL_CONFIG_PATH")
        if not path_to_try:
            default_paths = [
        ...
        if "SENTINEL_DATABASE_URL" in os.environ:
            config_dict.setdefault("storage", {})["database_url"] = os.environ["SENTINEL_DATABASE_URL"]

        if "SENTINEL_LLM_PROVIDER" in os.environ:
            config_dict.setdefault("elicitor", {})["provider"] = os.environ["SENTINEL_LLM_PROVIDER"]
