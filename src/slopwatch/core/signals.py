...
_CATALOG_ENTRIES: List[SignalSpec] = [
    ...
    _spec("MODULE_TOPLEVEL_EXECUTION", "Module-scope code execution on import", SignalCategory.MALWARE.value,
          Severity.CRITICAL.value, default_score=40, kind=FindingKind.OBSERVATION.value,
          ui_badge="⚡ Import Exec", ui_facet="has_install_hook",
          is_code_execution=True, gates_malicious=True,
          description="A dangerous call sits at module scope outside setup.py and runs automatically the moment anything imports the module — the same certainty as an install-time hook, just triggered by `import` instead of `pip install`."),
