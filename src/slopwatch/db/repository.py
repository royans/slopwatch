...
logger = logging.getLogger("slopwatch.repository")
...
from slopwatch.db.models import (
...
class SlopWatchRepository:
    ...
    async def bulk_sync_registered_packages(self, ecosystem: Ecosystem, package_names: Set[str]) -> int:
        ...
        """
        from slopwatch.core.brands import compute_brand_priority
    ...
    async def record_detection(self, detection: SquatDetection) -> SquatDetection:
        ...
        from slopwatch.core.freshness import calculate_next_audit_time
    ...
    async def _replace_findings(self, detection: SquatDetection) -> int:
        """
        Rewrite ``slopwatch_findings`` rows for one detection: delete the old set,
    ...
    async def find_detections_by_signals(
        ...
        *,
        ...
        codes. Runs entirely against the indexed ``slopwatch_findings`` table
    ...
    async def backfill_findings(self, batch_size: int = 500) -> Dict[str, int]:
        """
        Populate ``slopwatch_findings`` for every existing detection from its stored
    ...
    async def mark_grammar_matching_packages(
        ...
        entity+capability+framework naming grammar (slopwatch.matrix.generator), using
        the same candidate-generation + set-intersection approach as `audit-catalog`.
        Returns the number of newly-flagged rows.
        """
        from slopwatch.matrix.generator import generate_ecosystem_candidates
    ...
    async def get_taxonomy_gap_report(
        ...
        """
        from slopwatch.core.taxonomies import ENTITIES, CAPABILITIES, FRAMEWORKS
        from slopwatch.core.brands import PRIORITY_BRAND_WEIGHTS
    ...
    async def backfill_install_hook_flags(self, batch_size: int = 500) -> Dict[str, int]:
        ...
        """
        from slopwatch.assessor.scorer import has_install_time_code_execution, has_install_time_network_socket
    ...
    async def refresh_domain_reputations(self) -> int:
        ...
        for rep in reputations.values():
            batch.append({
                ...
                "is_generic_esp": not rep.is_custom_domain,
                "has_malware": rep.malicious_count > 0,
...
SentinelRepository = SlopWatchRepository
