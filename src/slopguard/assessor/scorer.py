...
from slopguard.core.taxonomies import (
    VENDOR_DOMAINS,
    PUBLIC_EMAIL_PROVIDERS,
    DISPOSABLE_EMAIL_DOMAINS,
)
from slopguard.core.signals import build_findings
...
# Flag prefixes emitted by the AST/manifest inspectors (slopguard.assessor.python_ast,
# slopguard.assessor.npm_manifest) that indicate the package CAN execute arbitrary code
...
class ProgressiveThreatEvaluator:
    def __init__(
        ...
        # Plugin detector engine (slopguard.detectors). Lazily resolved to the
    ...
    def _get_engine(self):
        if self._detection_engine is None:
            from slopguard.detectors.engine import get_shared_engine
    ...
    async def _run_plugin_detectors(
        ...
        *,
        ...
        try:
            from slopguard.detectors.base import PackageContext
            ctx = PackageContext(
    ...
    def verify_vendor_ownership(self, entity_token: str, author_email: Optional[str]) -> bool:
        ...
        if not author_email:
            return False
        from slopguard.core.normalizers import extract_clean_email_and_domain
    ...
    async def evaluate_candidate(
        ...
        async with self.semaphore:
            ...
            # ==================== 4b. PLUGIN DETECTORS (slopguard.detectors) ====================
