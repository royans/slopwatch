...
class StorageConfig(BaseModel):
    database_url: str = "sqlite+aiosqlite:///data/slopguard.db"
    wal_mode: bool = True
...
def _find_default_signatures_path() -> str:
    ...
    try:
        ...
        sig_pkg = pkg_resources.files("slopguard") / "signatures" / "parking_hashes.json"
        if sig_pkg.is_file():
            return str(sig_pkg)
    except Exception:
        pass
...
class Settings(BaseModel):
    ...
    def load(cls, config_path: Optional[str] = None) -> "Settings":
        ...
        path_to_try = config_path or os.getenv("SLOPGUARD_CONFIG_PATH") or os.getenv("SENTINEL_CONFIG_PATH")
        if not path_to_try:
            default_paths = [
        ...
        llm_provider = os.getenv("SLOPGUARD_LLM_PROVIDER") or os.getenv("SENTINEL_LLM_PROVIDER")
        if llm_provider:
