...
``Finding`` objects. Detectors live in ``slopguard.detectors.*`` and register
...
logger = logging.getLogger("slopguard.detectors")
...
def _discover_modules() -> None:
    """Import every submodule of ``slopguard.detectors`` so decorators run."""
    import slopguard.detectors as pkg

    for mod in pkgutil.iter_modules(pkg.__path__):
        if mod.name in ("base", "engine") or mod.name.startswith("_"):
            continue
        try:
            importlib.import_module(f"slopguard.detectors.{mod.name}")
...
def _config_disabled_detectors() -> Set[str]:
    ...
    candidates = [
        os.getenv("SLOPGUARD_DETECTORS_CONFIG"),
